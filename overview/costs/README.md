# Project Costs

Spent $0.00 of $0.00 USD. $0.00 remains overall and $0.00 remains before the 100% stop
threshold.

## Budget Summary

| Field | Value |
|-------|-------|
| Total budget | $0.00 USD |
| Total spent | $0.00 USD |
| Budget left | $0.00 USD |
| Budget left before stop threshold | $0.00 USD |
| Spent percent | 0.00% |
| Warn threshold | 80% ($0.00) |
| Stop threshold | 100% ($0.00) |
| Default per-task limit | $0.00 USD |
| Tasks with cost records | 3 |
| Tasks with non-zero spend | 0 |
| Skipped tasks | 2 |

## Service Totals

No entries found.

## Breakdown Totals

No entries found.

3 task cost record(s) are zero-cost and omitted from the main spend table.

## Task Spend

No task cost records found.

## Skipped Tasks

| Task ID | Reason |
|---------|--------|
| `t0004_generate_target_tuning_curve` | results/costs.json is missing or invalid |
| `t0005_download_dsgc_morphology` | results/costs.json is missing or invalid |
